...
- What happens when multiple pAInt instances try to inspect the same target? Each editor instance gets its own iframe with its own inspector context; messages are scoped to each iframe's parent window.
