...
**Goal**: Create the standalone inspector JavaScript file that developers add to their target project. When loaded inside the Dev Editor iframe, it initializes inspection capabilities and communicates with the editor. When loaded directly in a browser, it does nothing.

**Independent Test**: Add script tag to any localhost project, load in Dev Editor iframe, verify `INSPECTOR_READY` is sent and element selection works.
