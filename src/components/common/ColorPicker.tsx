...
function parseColor(value: string): { rgb: RGB; alpha: number } {
  ...
  if (rgbaMatch) {
    return {
      ...
      alpha: rgbaMatch[4] !== undefined ? Math.round(parseFloat(rgbaMatch[4]) * 100) : 100,
    };
  }
  // Handle hex
      ...
      {editing ? (
        ...
        <span
          ...
          onPointerMove={(e) => {
            ...
            const next = Math.max(min, Math.min(max, startVal.current + delta * mult));
...
export function ColorPicker({ value, onChange, onSelectVariable, label }: ColorPickerProps) {
  ...
  useEffect(() => {
    if (!isOpen) return;
    if (satCanvasRef.current) drawSaturationBrightness(satCanvasRef.current, hsv.h);
  }, [isOpen, hsv.h]);
  ...
  const satDrag = usePointerDrag(
    useCallback(
      (x: number, y: number, rect: DOMRect) => {
        ...
        const v = Math.round(Math.max(0, Math.min(100, 100 - (y / rect.height) * 100)));
        const next = { ...hsv, s, v };
        setHsv(next);
  ...
  // HSV numeric changes
  const updateH = useCallback((h: number) => {
    const next = { ...hsv, h };
    setHsv(next);
    setHexInput(rgbToHex(hsvToRgb(next)).slice(1));
    emit(next, alpha);
  ...
  const updateS = useCallback((s: number) => {
    const next = { ...hsv, s };
    setHsv(next);
    setHexInput(rgbToHex(hsvToRgb(next)).slice(1));
    emit(next, alpha);
  ...
  const updateV = useCallback((v: number) => {
    const next = { ...hsv, v };
    setHsv(next);
    setHexInput(rgbToHex(hsvToRgb(next)).slice(1));
    emit(next, alpha);
  ...
  const updateA = useCallback((a: number) => {
    setAlpha(a);
    emit(hsv, a);
  }, [hsv, emit]);
      ...
      <div className="flex items-center gap-1 flex-1 relative">
        ...
        {isOpen && (
          ...
          >
            ...
            <div className="relative mb-2 rounded overflow-hidden" style={{ height: '140px' }}>
              <canvas
                ref={(el) => {
                  (satDrag.ref as React.MutableRefObject<HTMLCanvasElement | null>).current = el;
                  (satCanvasRef as React.MutableRefObject<HTMLCanvasElement | null>).current = el;
                }}
                width={216}
                height={140}
            ...
            {/* Hue slider */}
            <div className="relative mb-1.5 rounded-full overflow-hidden" style={{ height: '10px' }}>
              <canvas
                ref={(el) => {
                  (hueDrag.ref as React.MutableRefObject<HTMLCanvasElement | null>).current = el;
                  (hueCanvasRef as React.MutableRefObject<HTMLCanvasElement | null>).current = el;
                }}
                width={216}
                height={10}
            ...
            {/* Alpha slider */}
            <div className="relative mb-2 rounded-full overflow-hidden" style={{ height: '10px' }}>
              <canvas
                ref={(el) => {
                  (alphaDrag.ref as React.MutableRefObject<HTMLCanvasElement | null>).current = el;
                  (alphaCanvasRef as React.MutableRefObject<HTMLCanvasElement | null>).current = el;
                }}
                width={216}
                height={10}
                className="w-full h-full cursor-pointer block"
            ...
            <div className="flex items-center justify-between px-1">
              <ScrubInput label="H" value={hsv.h} min={0} max={359} onChange={updateH} />
              <ScrubInput label="S" value={hsv.s} min={0} max={100} onChange={updateS} />
