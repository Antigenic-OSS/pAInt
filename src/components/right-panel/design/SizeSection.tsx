...
export function SizeSection() {
  ...
  const hasChanges = useEditorStore((s) => {
    ...
    if (!sp) return false;
    return s.styleChanges.some((c) => c.elementSelector === sp && SIZE_PROPERTIES.includes(c.property));
  });
  ...
  const handleResetAll = useCallback(() => {
    ...
    const matching = styleChanges.filter(
      (c) => c.elementSelector === selectorPath && SIZE_PROPERTIES.includes(c.property)
    );
