...
export function DraggableLabel({
  ...
  const handlePointerDown = useCallback(
    (e: React.PointerEvent) => {
      ...
      if (value === 'auto' || value === 'none' || value === 'normal' || isNaN(parsed.number)) return;
  ...
  const handlePointerMove = useCallback(
    (e: React.PointerEvent) => {
      ...
      const next = clamp(
        Math.round((dragStartValue.current + delta * step * multiplier) * 100) / 100
      );
  ...
  const parsed = parseCSSValue(value);
  const isDraggable = value !== 'auto' && value !== 'none' && value !== 'normal' && !isNaN(parsed.number);
