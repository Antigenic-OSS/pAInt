...
import type { ClaudeScanRequest, ClaudeScanResponse, ScanGroup, ProjectScanResult } from '@/types/claude';
...
function parseScanOutput(stdout: string): ClaudeScanResponse {
  ...
  if (jsonMatch) {
    try {
      ...
      return {
        smartPrompt: typeof parsed.smartPrompt === 'string' ? parsed.smartPrompt : stdout.trim(),
        intent: typeof parsed.intent === 'string' ? parsed.intent : 'Visual styling updates',
        groups: Array.isArray(parsed.groups) ? (parsed.groups as ScanGroup[]) : [],
 ...
 */
function parseAndValidateScan(body: ClaudeScanRequest & { projectScan?: ProjectScanResult }): { error: NextResponse } | { prompt: string; resolvedRoot: string } {
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    if (message === 'TIMEOUT') {
      return NextResponse.json({ error: 'Claude CLI timed out after 5 minutes' }, { status: 504 });
    }
