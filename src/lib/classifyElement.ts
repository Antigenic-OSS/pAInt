...
import type { FileMap, ComponentEntry, RouteEntry, SourceInfo } from '@/types/claude';
...
function matchesUrlPattern(urlPath: string, pattern: string): boolean {
  const urlSegs = urlPath.replace(/^\/|\/$/g, '').split('/').filter(Boolean);
  const patSegs = pattern.replace(/^\/|\/$/g, '').split('/').filter(Boolean);
  ...
  if (patSegs.length > 0) {
    ...
    if (/^\[{1,2}\.\.\./.test(last)) {
      ...
      for (let i = 0; i < patSegs.length - 1; i++) {
        ...
        if (!patSegs[i].startsWith('[') && patSegs[i] !== urlSegs[i]) return false;
      }
...
function findPageRoute(routes: RouteEntry[], pagePath: string): RouteEntry | null {
  const pageRoutes = routes.filter((r) => r.type === 'page');
  // Exact match first
  const exact = pageRoutes.find((r) => r.urlPattern === pagePath);
  if (exact) return exact;
  // Dynamic segment match
  const dynamic = pageRoutes.find((r) => matchesUrlPattern(pagePath, r.urlPattern));
...
function resolveFromFileMap(opts: {
  ...
  for (const comp of fileMap.components) {
    if (comp.category === 'page' || comp.category === 'screen' || comp.category === 'layout') continue;
  ...
  const isLayoutHint = LAYOUT_TAGS.has(tag) || matchesAny(combined, LAYOUT_HINTS);
  if (isLayoutHint && fileMap.routes.length > 0) {
    const layouts = fileMap.routes
      .filter((r) => r.type === 'layout')
      .filter((r) => matchesUrlPattern(pagePath, r.urlPattern) || r.urlPattern === '/');
    if (layouts.length > 0) {
  ...
  if (fileMap.routes.length > 0) {
    ...
    if (pageRoute) {
      const depth = opts.selectorPath ? opts.selectorPath.split(' > ').length : 0;
      if (depth <= 4 || matchesAny(combined, PAGE_HINTS)) return pageRoute.filePath;
    }
  }

  // 4. Screen/view match — look for screen/page entries with name matching URL segment
  const urlSegments = pagePath.replace(/^\/|\/$/g, '').split('/').filter(Boolean);
  if (urlSegments.length > 0) {
    const screenEntries = fileMap.components.filter(
      (c) => c.category === 'screen' || c.category === 'page' || c.category === 'view',
    );
  ...
  if (pagePath === '/' || pagePath === '') {
    ...
    const homeNames = ['index', 'home', 'main', 'app', 'homescreen', 'homeview', 'homepage', 'mainscreen'];
  ...
  if (fileMap.routes.length > 0) {
    const rootPage = fileMap.routes.find((r) => r.type === 'page' && r.urlPattern === '/');
...
export function inferComponentWidgetName(opts: {
  ...
  if (opts.sourceInfo?.componentChain?.length) {
    return opts.sourceInfo.componentChain[opts.sourceInfo.componentChain.length - 1];
  }
