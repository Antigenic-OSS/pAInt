...
async function walkDir(
  ...
  for await (const [name, entryHandle] of dirHandle.entries()) {
    ...
    if (entryHandle.kind === 'directory') {
      ...
      await walkDir(entryHandle as FileSystemDirectoryHandle, childPath, collectors, counter);
      continue;
    }
...
export async function scanProjectClient(
  ...
  for (const dir of candidateDirs) {
    const alreadyCovered = scannedRoots.some((root) => dir.startsWith(root + '/'));
  ...
  try {
    ...
    for await (const [name, entry] of publicHandle.entries()) {
      if (entry.kind === 'directory' && ASSET_DIR_NAMES.has(name.toLowerCase())) {
