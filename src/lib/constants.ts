...
export function buildInstructionsFooter(
  ...
  if (framework === 'flutter') {
    ...
    lines.push('Flutter does NOT use CSS. Styles are applied via widget properties and style objects.');
    lines.push('');
    lines.push('**Spacing**');
    lines.push('- padding → EdgeInsets.all(n), EdgeInsets.symmetric(horizontal: n, vertical: n), EdgeInsets.only(left: n, top: n, ...)');
    lines.push('- margin → wrap in Padding widget or use Container(margin: EdgeInsets.all(n))');
    lines.push('- gap (in Row/Column) → use SizedBox(width: n) or SizedBox(height: n) between children, or MainAxisAlignment.spaceEvenly');
    lines.push('');
    lines.push('**Typography**');
    lines.push('- font-size → TextStyle(fontSize: n)');
    lines.push('- font-weight → TextStyle(fontWeight: FontWeight.w400/w500/w600/w700/bold)');
    lines.push('- line-height → TextStyle(height: n) where n is multiplier (1.5 = 150%)');
    lines.push('- color → TextStyle(color: Color(0xFF______)) or Colors.blue');
    lines.push('- text-align → Text("...", textAlign: TextAlign.center/left/right)');
    lines.push('');
    lines.push('**Colors**');
    lines.push('- background-color → Container(color: Color(0xFF______)) or ColoredBox');
    lines.push('- border-color → BoxDecoration(border: Border.all(color: Color(0xFF______)))');
    lines.push('- Use Color(0xAARRGGBB) for hex, Colors.blue for Material palette');
    lines.push('');
    lines.push('**Layout**');
    lines.push('- display: flex → Row (horizontal) or Column (vertical)');
    lines.push('- justify-content → MainAxisAlignment.start/center/end/spaceBetween/spaceAround/spaceEvenly');
    ...
    lines.push('');
    lines.push('**Sizing**');
    lines.push('- width/height → SizedBox(width: n, height: n) or Container(width: n, height: n)');
    lines.push('- width: 100% → double.infinity');
    lines.push('- max-width → ConstrainedBox(constraints: BoxConstraints(maxWidth: n))');
    lines.push('');
    lines.push('**Borders**');
    lines.push('- border-radius → BoxDecoration(borderRadius: BorderRadius.circular(n))');
    lines.push('- border-width → BoxDecoration(border: Border.all(width: n))');
    lines.push('');
    lines.push('**Effects**');
    lines.push('- opacity → Opacity(opacity: 0.5, child: ...)');
    lines.push('- box-shadow → BoxDecoration(boxShadow: [BoxShadow(blurRadius: n, offset: Offset(x, y))])');

  } else if (framework === 'react-native') {
    // React Native-specific guidance
    lines.push('');
    lines.push('### React Native Styling');
    lines.push('React Native does NOT use CSS classes or Tailwind. Styles are applied via the `style` prop with StyleSheet objects.');
    lines.push('');
    lines.push('**How to Apply**');
    lines.push('Find the component and update its StyleSheet.create() definitions or inline style prop.');
    lines.push('```');
    ...
    lines.push('- padding → padding, paddingHorizontal, paddingVertical, paddingTop, paddingRight, paddingBottom, paddingLeft');
    lines.push('- margin → margin, marginHorizontal, marginVertical, marginTop, marginRight, marginBottom, marginLeft');
    ...
    lines.push('');
    lines.push('**Layout** — Flexbox by default (flex-direction defaults to column, not row)');
    lines.push('- display: flex → display: "flex" (default, usually omitted)');
    lines.push('- flex-direction → flexDirection: "row"/"column" (NOTE: default is "column", opposite of web)');
    lines.push('- justify-content → justifyContent: "flex-start"/"center"/"flex-end"/"space-between"/"space-around"');
    lines.push('- align-items → alignItems: "flex-start"/"center"/"flex-end"/"stretch"');
    lines.push('- flex: 1 → flex: 1');
    lines.push('');
    lines.push('**Sizing**');
    lines.push('- width/height → width: n / height: n (number for dp, "100%" for percentage)');
    lines.push('');
    lines.push('**Borders**');
    ...
    lines.push('- box-shadow → iOS: shadowColor/shadowOffset/shadowOpacity/shadowRadius; Android: elevation: n');

  } else if (usesTailwind) {
    // Tailwind CSS guidance (web frameworks)
    lines.push('');
    lines.push('### How to Apply in React / Next.js');
    lines.push('Styles in React are applied via the `className` prop on JSX elements.');
    lines.push('Find the component that renders the matching element and edit its className.');
    lines.push('');
    lines.push('### Tailwind CSS Mappings');
    lines.push('This project uses Tailwind CSS. Update utility classes in className — never add inline styles.');
    lines.push('');
    lines.push('**Spacing** (Tailwind uses a 4px = 1 unit scale: 4px=1, 8px=2, 12px=3, 16px=4, 20px=5, 24px=6, 32px=8, 40px=10, 48px=12, 64px=16)');
    lines.push('- padding → p-{n}, px-{n}, py-{n}, pt-{n}, pr-{n}, pb-{n}, pl-{n}');
    lines.push('- margin → m-{n}, mx-{n}, my-{n}, mt-{n}, mr-{n}, mb-{n}, ml-{n} (supports negative: -mt-4)');
    lines.push('- gap → gap-{n}, gap-x-{n}, gap-y-{n}');
    lines.push('');
    lines.push('**Typography**');
    lines.push('- font-size → text-xs(12px), text-sm(14px), text-base(16px), text-lg(18px), text-xl(20px), text-2xl(24px), text-3xl(30px), text-4xl(36px)');
    lines.push('- font-weight → font-thin(100), font-light(300), font-normal(400), font-medium(500), font-semibold(600), font-bold(700), font-extrabold(800)');
    lines.push('- line-height → leading-none(1), leading-tight(1.25), leading-snug(1.375), leading-normal(1.5), leading-relaxed(1.625), leading-loose(2)');
    lines.push('- letter-spacing → tracking-tighter(-0.05em), tracking-tight(-0.025em), tracking-normal(0), tracking-wide(0.025em), tracking-wider(0.05em)');
    lines.push('- text-align → text-left, text-center, text-right, text-justify');
    lines.push('- text-transform → uppercase, lowercase, capitalize, normal-case');
    lines.push('- text-decoration → underline, line-through, no-underline');
    lines.push('');
    lines.push('**Colors**');
    lines.push('- color → text-{color}-{shade} (text-gray-500, text-blue-600, text-red-500)');
    lines.push('- background-color → bg-{color}-{shade} (bg-white, bg-gray-100, bg-blue-500)');
    lines.push('- border-color → border-{color}-{shade}');
    lines.push('- For hex/rgb values not in the palette, use arbitrary: text-[#1a1a1a], bg-[rgb(30,30,30)]');
    lines.push('');
    lines.push('**Layout**');
    lines.push('- display: flex → flex, display: grid → grid, display: none → hidden, display: block → block, display: inline-flex → inline-flex');
    lines.push('- flex-direction → flex-row, flex-col, flex-row-reverse, flex-col-reverse');
    lines.push('- justify-content → justify-start, justify-center, justify-end, justify-between, justify-around, justify-evenly');
    lines.push('- align-items → items-start, items-center, items-end, items-baseline, items-stretch');
    ...
    lines.push('');
    lines.push('**Sizing**');
    lines.push('- width → w-{n}(spacing scale), w-full(100%), w-screen(100vw), w-auto, w-1/2(50%), w-1/3(33.3%), w-fit');
    lines.push('- height → h-{n}, h-full, h-screen, h-auto, h-fit');
    lines.push('- max-width → max-w-sm(384px), max-w-md(448px), max-w-lg(512px), max-w-xl(576px), max-w-2xl(672px), max-w-full, max-w-none');
    lines.push('- min-height → min-h-0, min-h-full, min-h-screen');
    lines.push('');
    lines.push('**Borders**');
    lines.push('- border-width → border(1px), border-0, border-2(2px), border-4(4px), border-t, border-b, border-l, border-r');
    lines.push('- border-radius → rounded-none(0), rounded-sm(2px), rounded(4px), rounded-md(6px), rounded-lg(8px), rounded-xl(12px), rounded-2xl(16px), rounded-full(9999px)');
    lines.push('- border-style → border-solid, border-dashed, border-dotted, border-none');
    lines.push('');
    lines.push('**Effects**');
    lines.push('- opacity → opacity-0, opacity-25, opacity-50, opacity-75, opacity-100');
    lines.push('- box-shadow → shadow-sm, shadow, shadow-md, shadow-lg, shadow-xl, shadow-2xl, shadow-none');
    lines.push('');
    lines.push('**Position**');
    lines.push('- position → static, relative, absolute, fixed, sticky');
    lines.push('- top/right/bottom/left → top-{n}, right-{n}, bottom-{n}, left-{n} (uses spacing scale), inset-0(all sides 0)');
    lines.push('- z-index → z-0, z-10, z-20, z-30, z-40, z-50, z-auto');
    lines.push('');
    lines.push('**Arbitrary values**: When no Tailwind scale value matches, use bracket syntax: w-[200px], mt-[13px], text-[15px], bg-[#1e1e1e], grid-cols-[1fr_2fr]');
    lines.push('');
    lines.push('### Responsive Breakpoints');
    lines.push('Tailwind uses mobile-first breakpoint prefixes. Unprefixed classes apply to all sizes.');
    lines.push('- sm: → 640px+, md: → 768px+, lg: → 1024px+, xl: → 1280px+, 2xl: → 1536px+');
    ...
    lines.push('- Example: className="text-sm md:text-base lg:text-lg" (small on mobile, base on tablet, large on desktop)');

  } else {
    // Generic web CSS guidance (no Tailwind detected, or unknown framework)
    lines.push('');
    lines.push('### How to Apply');
    lines.push('Find the element matching the selector and update its styles using the');
    lines.push('project\'s CSS approach (CSS files, CSS modules, styled-components, inline styles, etc.).');
    lines.push('');
    lines.push('If the project uses CSS classes, update the class definitions.');
    lines.push('If the project uses inline styles in JSX (`style={{ }}`), update the style object.');
    lines.push('');
    lines.push('### Responsive Breakpoints');
    lines.push('- Changes marked [desktop] apply at wider viewports (typically @media (min-width: 1024px))');
    lines.push('- Changes marked [tablet] apply at medium viewports (typically @media (min-width: 768px))');
    lines.push('- Changes marked [mobile] apply at the base / smallest viewports');
  }
