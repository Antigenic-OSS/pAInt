...
const SCRIPT_STRIP_RE = /<script\b(?![^>]*type\s*=\s*["']application\/ld\+json["'])[^>]*>[\s\S]*?<\/script\s*>/gi;
const SCRIPT_SELF_CLOSING_RE = /<script\b(?![^>]*type\s*=\s*["']application\/ld\+json["'])[^>]*\/\s*>/gi;
...
function rewriteCssUrls(css: string, targetOrigin: string): string {
  ...
  return css.replace(
    new RegExp(`url\\(\\s*['"]?${targetOrigin.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(/[^'")\\s]*)['"]?\\s*\\)`, 'g'),
    'url($1)',
  );
}
...
export async function handleProxy(
  ...
  if (!targetUrl) {
    return Response.json(
      { error: 'No target URL. Add ?x-dev-editor-target=http://localhost:PORT to your first request.' },
  ...
  try {
    ...
    if (contentType && (contentType.includes('font') || pathname.match(/\.(woff2?|ttf|otf|eot)$/i))) {
