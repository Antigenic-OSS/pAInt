...
import { getClaudeBin, spawnClaude, spawnClaudeStreaming, isAuthError } from '../lib/claude-bin';
...
async function handleProjectScan(req: Request, cors: Record<string, string>): Promise<Response> {
  if (req.method !== 'POST') {
  ...
  if (!projectRoot || typeof projectRoot !== 'string') {
    return json({ error: 'projectRoot is required' }, { status: 400, headers: cors });
  }
...
const FRAMEWORK_PREFIXES = [
  '--tw-', '--next-', '--radix-', '--chakra-',
  '--mantine-', '--mui-', '--framer-', '--sb-', '--css-interop-',
];
...
function walkForCSS(
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) { walkForCSS(fullPath, projectRoot, results, counter); continue; }
    ...
    while ((match = CSS_VAR_RE.exec(content)) !== null) {
      ...
      if (!results.has(name)) {
        results.set(name, { value: rawValue, resolvedValue: rawValue, selector: ':root', source: relativePath });
      }
    }
  }
}

async function handleCSSVariablesScan(req: Request, cors: Record<string, string>): Promise<Response> {
  if (req.method !== 'POST') return json({ error: 'Method not allowed' }, { status: 405, headers: cors });

  let body: { projectRoot?: string };
  try { body = await req.json(); } catch { return json({ error: 'Invalid JSON body' }, { status: 400, headers: cors }); }

  const { projectRoot } = body;
  if (!projectRoot || typeof projectRoot !== 'string') return json({ error: 'projectRoot is required' }, { status: 400, headers: cors });

  const rootError = validateProjectRoot(projectRoot);
  if (rootError) return json({ error: rootError }, { status: 400, headers: cors });
  ...
  try {
    ...
    for (const [name, def] of cssResults) {
      definitions[name] = { value: def.value, resolvedValue: def.resolvedValue, selector: def.selector };
    }

    return json({ definitions, count: Object.keys(definitions).length, filesScanned: counter.count }, { headers: cors });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return json({ error: 'Scan failed', details: message }, { status: 500, headers: cors });
  }
}

// ─── Tailwind Config Scan ───────────────────────────────────

const TW_CONFIG_FILES = ['tailwind.config.ts', 'tailwind.config.js', 'tailwind.config.mjs', 'tailwind.config.cjs'];
...
function parseColorBlock(
  ...
  while ((nestedMatch = nestedRe.exec(body)) !== null) {
    ...
    const nestedPrefix = prefix ? `${prefix}-${kebab(nestedMatch[1])}` : kebab(nestedMatch[1]);
  ...
  while ((entryMatch = entryRe.exec(body)) !== null) {
    ...
    results[varName] = { value, resolvedValue: value, selector: `tailwind:${filePath}` };
  }
}

async function handleTailwindConfigScan(req: Request, cors: Record<string, string>): Promise<Response> {
  if (req.method !== 'POST') return json({ error: 'Method not allowed' }, { status: 405, headers: cors });
  ...
  if (!configPath) return json({ definitions: {}, count: 0, found: false }, { headers: cors });

  try {
    const raw = readFileSync(configPath);
    if (raw.length > MAX_CONFIG_SIZE) return json({ error: 'Config file too large' }, { status: 400, headers: cors });
    ...
    return json({ definitions, count: Object.keys(definitions).length, found: true, configFile: relativePath }, { headers: cors });
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Unknown error';
    return json({ error: 'Parse failed', details: message }, { status: 500, headers: cors });
  }
}

// ─── Claude Status ───────────────────────────────────────────

async function handleClaudeStatus(req: Request, cors: Record<string, string>): Promise<Response> {
  if (req.method === 'GET') {
    try {
      const claudeBin = getClaudeBin();
      const { stdout } = await execFileAsync(claudeBin, ['--version'], { timeout: 10_000 });
      return json({ available: true, version: stdout.trim() }, { headers: cors });
    } catch (err) {
      const msg = err instanceof Error ? err.message : String(err);
      return json({ available: false, error: `claude CLI not found: ${msg.slice(0, 200)}` }, { headers: cors });
    }
  }
...
function parseDiffs(output: string) {
  const diffs: Array<{ filePath: string; hunks: Array<{ header: string; lines: Array<{ type: string; content: string }> }>; linesAdded: number; linesRemoved: number }> = [];
  ...
  for (const line of lines) {
    if (line.startsWith('+++ b/') || line.startsWith('+++ ')) {
      const filePath = line.startsWith('+++ b/') ? line.slice(6).trim() : line.slice(4).trim();
...
function buildSummary(diffs: ReturnType<typeof parseDiffs>): string {
  if (diffs.length === 0) return 'No file changes detected in the analysis output.';
...
async function handleClaudeAnalyze(req: Request, cors: Record<string, string>): Promise<Response> {
  if (req.method !== 'POST') {
    return json({ error: 'Method not allowed' }, { status: 405, headers: cors });
  }
  ...
  if (!hasSmartPrompt && (!changelog || typeof changelog !== 'string' || changelog.length === 0)) {
    return json({ error: 'changelog is required' }, { status: 400, headers: cors });
  }
  if (!projectRoot || typeof projectRoot !== 'string') {
    return json({ error: 'projectRoot is required' }, { status: 400, headers: cors });
  }
    ...
    const stream = new ReadableStream({
      start(controller) {
        ...
        spawnClaudeStreaming(
          ...
          { cwd: resolvedRoot, timeout: TIMEOUT_MS, onStderr: (line) => enqueue('stderr', { line }) },
        )
          .then((result) => {
          ...
          .catch((err) => {
            ...
            enqueue('error', { code: message === 'TIMEOUT' ? 'TIMEOUT' : 'SPAWN_ERROR', message });
          })
          .finally(() => {
            enqueue('done', {});
            controller.close();
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      ...
      if (isAuthError(stderr)) {
        return json({ error: 'Claude CLI is not authenticated. Run `claude login` in your terminal.', authRequired: true }, { status: 401, headers: cors });
      }
      return json({ error: 'Claude CLI exited with an error', details: stderr || 'Unknown CLI error' }, { status: 500, headers: cors });
    }
    ...
    const message = error instanceof Error ? error.message : 'Unknown error';
    if (message === 'TIMEOUT') {
      return json({ error: 'Claude CLI timed out after 120 seconds' }, { status: 504, headers: cors });
    }
    return json({ error: 'Failed to run Claude CLI', details: message }, { status: 500, headers: cors });
  }
}
...
async function handleClaudeApply(req: Request, cors: Record<string, string>): Promise<Response> {
  if (req.method !== 'POST') {
    return json({ error: 'Method not allowed' }, { status: 405, headers: cors });
  }
  ...
  if (!sessionId || typeof sessionId !== 'string') {
    return json({ error: 'sessionId is required' }, { status: 400, headers: cors });
  }
  if (!/^[a-f0-9-]+$/i.test(sessionId)) {
    return json({ error: 'sessionId contains invalid characters' }, { status: 400, headers: cors });
  }
  if (!projectRoot || typeof projectRoot !== 'string') {
    return json({ error: 'projectRoot is required' }, { status: 400, headers: cors });
  }
  ...
  try {
    ...
    if (result.exitCode !== 0) {
      return json(
        { error: 'Claude CLI exited with an error', details: result.stderr.trim() || 'Unknown CLI error' },
...
async function handlePickFolder(cors: Record<string, string>): Promise<Response> {
  ...
  try {
    ...
    if (platform === 'darwin') {
      const { stdout } = await execFileAsync('osascript', [
        '-e', 'set theFolder to POSIX path of (choose folder with prompt "Select your project root folder")',
        '-e', 'return theFolder',
      ], { timeout: 60_000 });
      ...
      return json({ error: 'Folder picker not supported on this platform' }, { status: 501, headers: cors });
    }
    ...
    return json({ error: 'Failed to open folder picker' }, { status: 500, headers: cors });
  }
}

// ─── Main Router ─────────────────────────────────────────────

export async function handleAPI(req: Request, url: URL, cors: Record<string, string>): Promise<Response> {
