...
When a user selects an element in pAInt preview, the right panel shows SOURCE (page/layout file) and COMPONENT (the React component file). Currently, COMPONENT is always empty in proxy mode because React fibers aren't available (scripts are stripped to prevent reload loops).
